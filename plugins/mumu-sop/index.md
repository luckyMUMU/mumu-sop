...
| Skill | 触发词 | 描述 | 输入 | 输出 |
|-------|--------|------|------|------|
| [sop-orchestrator](./sop-orchestrator/SKILL.md) | `$start`, `$workflow` | **默认入口**：编排工作流程 | 权重决策、宪章文档 | 工作流状态文件 |
| [sop-sync](./sop-sync/SKILL.md) | `$sync`, `$document` | 同步文档与代码 | 代码变更、设计文档 | 文档更新列表 |
...
| Skill | 触发词 | 描述 | 输入 | 输出 |
|-------|--------|------|------|------|
| [sop-decision-analyst](./sop-decision-analyst/SKILL.md) | `$decision`, `$analyze` | **前置决策**：复杂需求分析 | 用户请求、上下文 | 意图分析、决策路径、执行计划 |
| [sop-requirement-analyst](./sop-requirement-analyst/SKILL.md) | `$requirement`, `$spec` | 分析需求生成规范 | 需求描述、现有规范 | 规范文档、BDD场景 |
| [sop-architecture-designer](./sop-architecture-designer/SKILL.md) | `$architecture`, `$design` | 系统架构设计 | 系统需求、宪章文档 | 架构设计文档 |
...
| [sop-code-implementer](./sop-code-implementer/SKILL.md) | `$implement`, `$code` | 根据规范实现代码 | 设计文档、规范文档 | 代码变更 |
| [sop-test-implementer](./sop-test-implementer/SKILL.md) | `$test`, `$tdd` | 根据BDD场景编写测试 | 规范文档、BDD场景 | 测试代码 |
...
| [sop-code-reviewer](./sop-code-reviewer/SKILL.md) | `$review`, `$audit` | 审查代码实现 | 代码变更、测试报告 | 代码审查报告 |
| [sop-spring-reviewer](./sop-spring-reviewer/SKILL.md) ⚠️ | `$spring-review`, `$java-review` | **[Java/Spring专用]** Spring审查 | 代码变更 | Spring审查报告 |
...
| Skill | 触发词 | 描述 | 输入 | 输出 |
|-------|--------|------|------|------|
| [sop-bug-analyst](./sop-bug-analyst/SKILL.md) | `$bug`, `$analyze` | Bug分析与根因定位 | Bug描述、复现步骤 | 复现测试、分析报告 |
| [sop-code-refactorer](./sop-code-refactorer/SKILL.md) | `$refactor`, `$clean` | 安全重构代码 | 重构目标、代码位置 | 重构后代码、变更报告 |
...
| Skill | 触发词 | 描述 | 输入 | 输出 |
|-------|--------|------|------|------|
| [sop-document-writer](./sop-document-writer/SKILL.md) | `$doc`, `$create-doc`, `$user-guide` | 创建文档（技术/人类双模式） | 文档类型、目标读者 | 规范文档、用户手册 |
