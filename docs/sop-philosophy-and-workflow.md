...
2. [核心理念](#2-核心理念)
   - [2.1 约束树驱动](#21-约束树驱动)
   - [2.2 设计从根开始](#22-设计从根开始)
   - [2.3 实现从叶子开始](#23-实现从叶子开始)
   - [2.4 独立任务树上下文](#24-独立任务树上下文)
   - [2.5 契约式协作](#25-契约式协作)
3. [约束树架构](#3-约束树架构)
...
6. [执行效果](#6-执行效果)
7. [最佳实践](#7-最佳实践)
   - [7.3 独立上下文执行](#73-独立上下文执行)
   - [7.5 Skill 使用](#75-skill-使用)
   - [7.6 常见陷阱](#76-常见陷阱)
...
**理念**: 在实现阶段，每个节点拥有独立的任务树上下文，上下文之间通过摘要传递信息。
...
| 规则 | 描述 |
|------|------|
| **上下文隔离** | 每个节点的任务树在独立上下文中执行，不共享状态 |
| **摘要传递** | 子节点完成后创建摘要，传递给父节点上下文 |
| **顺序执行** | 必须完成所有子节点任务后，才能执行父节点任务 |
| **摘要内容** | 包含变更内容、验证结果、遗留问题、依赖信息 |

**摘要结构**:

```yaml
node_summary:
  node_id: "P3-impl-user-auth"
  status: completed | partial | blocked

  changes:
    - type: code | config | doc
      path: "src/auth/login.ts"
      description: "实现用户登录逻辑"
  ...
  issues:
    - id: ISSUE-001
      severity: warning
      description: "需要补充边界测试"

  dependencies:
    - node: "P3-impl-session"
      status: completed
      impact: "依赖会话管理模块"
...
设计阶段（自顶向下）:
═════════════════════════════════════════════════════════════════
P0 设计 ───▶ P1 设计 ───▶ P2 设计 ───▶ P3 设计 ───▶ 临时节点设计
   │            │            │            │              │
   │  变更传递   │  变更传递   │  变更传递   │   变更传递    │
   ▼            ▼            ▼            ▼              ▼
  完成         完成         完成         完成           完成
...
| 步骤 | 上下文 | 输入 | 任务 | 输出 |
|------|--------|------|------|------|
| 12 | Temp-Context | 设计文档 | 临时节点任务树 | Temp-Summary |
| 13 | P3-Context | Temp-Summary | P3 任务树 | P3-Summary |
| 14 | P2-Context | P3-Summary | P2 任务树 | P2-Summary |
| 15 | P1-Context | P2-Summary | P1 任务树 | P1-Summary |
| 16 | P0-Context | P1-Summary | P0 验证任务树 | 验证报告 |

**上下文执行规则**:

1. **上下文创建**: 进入节点前创建独立上下文
2. **任务树加载**: 从该节点的设计文档加载任务树
3. **摘要接收**: 接收子节点传递的摘要（如有）
4. **任务执行**: 在独立上下文中执行所有任务
5. **护栏检查**: 任务完成后执行护栏验证
6. **摘要生成**: 生成当前节点的摘要
7. **上下文销毁**: 传递摘要后销毁当前上下文
...
| 实践 | 描述 |
|------|------|
| 上下文隔离 | 每个节点在独立上下文中执行，不共享状态 |
| 完整叶子任务 | 必须完成所有子节点任务后才能执行父节点任务 |
| 摘要生成 | 每个节点完成后生成摘要，记录变更和验证结果 |
| 摘要传递 | 将子节点摘要传递给父节点上下文，确保信息完整 |
| 上下文销毁 | 摘要传递后销毁当前上下文，释放资源 |

**上下文执行检查清单**:

```yaml
context_execution_checklist:
  before_execution:
    - 确认所有子节点任务已完成
    - 确认子节点摘要已生成
    - 创建新的独立上下文
...
| **上下文状态污染** | 每个节点在独立上下文执行，禁止跨上下文状态访问 |
| **摘要传递缺失** | 子节点必须生成摘要，父节点必须接收摘要 |
| **执行顺序错误** | 必须完成所有子节点任务后才能执行父节点任务 |
| **上下文未销毁** | 摘要传递后必须销毁上下文，避免资源泄漏 |
